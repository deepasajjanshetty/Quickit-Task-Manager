<template>
  ...
  <div class="space-y-6">
    <div class="bg-black rounded-3xl shadow-sm hover:shadow-md transition-shadow">
        ...
        <div class="flex justify-between items-start">
          <div >
            <nav class="flex items-center space-x-2 text-sm text-blue-100 mb-1">
              ...
              <span>/</span>
              <span class="text-white">Web Projects</span>
            </nav>
          ...
          <div class="flex gap-6">
            <div>
              ...
              <div class="relative">
                ...
                <!-- Priority Dropdown -->
                <div v-if="showPriorityDropdown" class="absolute top-full left-0 mt-1 bg-gray-800 border border-gray-700 rounded-lg shadow-lg min-w-[120px] z-[9999]">
                  <div class="p-1">
                    <div 
            ...
            <div>
              ...
              <div class="flex gap-1 flex-wrap">
                ...
                <button 
                  @click="selectTag('indigo')" 
                  :class="[
                    'text-xs px-2 py-1 rounded-sm transition-colors cursor-pointer',
                    selectedTag === 'indigo' 
                      ? 'bg-indigo-500 text-white' 
                      : 'bg-indigo-500/50 text-indigo-300 hover:bg-indigo-500/70'
                  ]"
                >
                  Frontend
                </button>
                <button 
                  @click="selectTag('pink')" 
                  :class="[
                    'text-xs px-2 py-1 rounded-sm transition-colors cursor-pointer',
                    selectedTag === 'pink' 
                      ? 'bg-pink-500 text-white' 
                      : 'bg-pink-500/50 text-pink-300 hover:bg-pink-500/70'
                  ]"
                >
                  Backend
                </button>
          ...
          <div>
            <div class="flex items-center gap-3">
              ...
              <div class="w-6 h-6  border-2 border-gray-300 rounded-full flex items-center justify-center">
                <span class="text-gray-700 text-white text-xs font-medium">5</span>
              </div>
              ...
              <div class="flex -space-x-2">
                <div class="w-8 h-8 bg-blue-400 rounded-full border-2 border-gray-800 flex items-center justify-center">
                  <span class="text-white text-xs font-medium">AC</span>
                </div>
                ...
                <div class="w-8 h-8 bg-yellow-400 rounded-full border-2 border-gray-800 flex items-center justify-center">
          ...
          <div class="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-4 border border-gray-700/50">
            ...
            <div class="space-y-3">
              <div 
                ...
                :key="task.id"
                :class="`bg-${task.dateColor}-500/20 border border-${task.dateColor}-500/30 rounded-xl p-4 cursor-pointer hover:bg-${task.dateColor}-500/30 transition-all`"
              >
                <div class="flex items-start justify-between mb-3">
                  <div class="flex items-center gap-2">
                    <i class="bx bx-calendar" :class="`text-${task.dateColor}-400 text-sm`"></i>
                    <span :class="`text-${task.dateColor}-300 text-sm`">{{ task.date }}</span>
                  </div>
                  <i class="bx bx-right-arrow-alt text-gray-400 text-sm"></i>
                </div>
                <h4 class="text-white font-medium mb-2">{{ task.title }}</h4>
...
const currentProjectTitle = ref('Web Projects')
...
// Task data with priority information and date ranges for Web Projects
const allTasks = ref({
  backlog: [
    {
      id: 1,
    ...
    },
    {
      ...
      priority: 'High',
      date: 'Mar 01, 2025',
      dateColor: 'gray', // Other
      members: ['AC', 'SM'],
      memberColors: ['blue-400', 'purple-400'],
    ...
    },
    {
      ...
      priority: 'High',
      date: 'Apr 15, 2025',
      dateColor: 'indigo', // Frontend
      members: ['AC', 'MJ'],
      memberColors: ['blue-400', 'yellow-400'],
  ...
  inProgress: [
    ...
    },
    {
      ...
      dateColor: 'purple', // UI/UX
      members: ['SM', 'MJ'],
      memberColors: ['purple-400', 'yellow-400'],
...
const getTaskCardClass = (dateColor) => {
  switch (dateColor) {
    case 'indigo': // Frontend
      return 'bg-indigo-500/20 border border-indigo-500/30 rounded-xl p-4 cursor-pointer hover:bg-indigo-500/30 transition-all'
    case 'pink': // Backend
      return 'bg-pink-500/20 border border-pink-500/30 rounded-xl p-4 cursor-pointer hover:bg-pink-500/30 transition-all'
    case 'purple': // UI/UX
...
const getTaskIconClass = (dateColor) => {
  switch (dateColor) {
    case 'indigo': // Frontend
      return 'text-indigo-400 text-sm'
    case 'pink': // Backend
      return 'text-pink-400 text-sm'
    case 'purple': // UI/UX
...
const getTaskDateClass = (dateColor) => {
  switch (dateColor) {
    case 'indigo': // Frontend
      return 'text-indigo-300 text-sm'
    case 'pink': // Backend
      return 'text-pink-300 text-sm'
    case 'purple': // UI/UX
